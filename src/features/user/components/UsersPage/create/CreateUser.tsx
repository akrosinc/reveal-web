...
import { createUser, createInstanceUser } from '../../../api';
...
const CreateUser = ({ show, handleClose }: Props) => {
  ...
  useEffect(() => {
    if (show && isAuthorized) {
      getInstances(1000, 0).then(res => {
        setInstances(res.content.map(inst => ({
  ...
  const submitHandler = (formValues: RegisterValues) => {
    if (isAuthorized) {
      ...
      toast.promise(createInstanceUser(instanceUserPayload), {
        ...
        success: {
          render() {
            ...
            handleClose();
            return `Instance User ${instanceUserPayload.username} created successfully.`;
          }
        },
        error: {
          render({ data: err }: { data: any }) {
             return err?.message || 'Failed to create instance user';
          }
        }
      });
      return;
