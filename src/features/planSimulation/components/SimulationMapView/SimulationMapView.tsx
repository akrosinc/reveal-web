...
const SimulationMapView = ({
  ...
  useEffect(() => {
    if (datasetsDataMap && map && map.current && state.datasets && state.datasets.length !== 0) {
      ...
      const matchingLayers = layers?.filter(
        layer => layer.id.startsWith(`ds-`) && (!layer.id.endsWith(selectedLoaction.properties.name) || showDatasetsAgainstParentLevel)
      );
  ...
  useEffect(() => {
    if (map && map.current && currentLocationChildren && selectedLoaction) {
      if (!showDatasetsAgainstParentLevel) map.current?.fitBounds(JSON.parse(JSON.stringify(bbox(selectedLoaction.geometry))));
      ...
      if (!map.current.getLayer('children-layer')) {
        ...
        const paintConfig = {
          ...
          'fill-outline-color': 'rgba(255, 000, 000, 0.8)'
        };
