...
const REPORT_TYPE = [
  ...
  {
    name: 'drugInventoryAdverseReporting',
    label: 'Drug Inventory / Adverse Reporting',
    value: 'DRUG_DISTRIBUTION'
  },
...
const Report = () => {
  ...
  const [showMapRepInfo, setShowMapRepInfo] = useState<boolean>(true);
        ...
        if (planId && reportType) {
          getReportTypeInfo(reportType).then(res => {
            ...
            Promise.all([
              ...
              getMapReportData(
                  {
                    parentLocationIdentifier: parentLocationId ? parentLocationId : null,
  ...
  useEffect(() => {
    if (showGraphs) {
      let data1:ChartData<'line'> = {
        // labels: [2000, 2005, 2010, 2015, 2020], // X-axis labels
        datasets: [
          {
            label: 'Sales',
            data: [{x: 2000, y: 20}, {x: 2005, y: 30}, {x: 2010, y: 15}, {x: 2015, y: 25}], // Y-axis data
            fill: false,
            borderColor: '#FF0000',
            tension: 0,
            pointStyle: 'rectRot',
          ...
          {
            label: 'Sales2',
            data: [{x: 2000, y: 14}, {x: 2005, y: 12}, {x: 2010, y: 5}, {x: 2015, y: 0}], // Y-axis data
            fill: true,
            borderDash: [5, 5],
          ...
          legend: {
            ...
            labels: {
              filter: (legendItem: LegendItem, chart: ChartData<'line'>) => {
                // `legendItem.datasetIndex` is the index of the dataset
                // Hide legend entry for dataset index 1 (the second dataset)
  ...
  return (
      <Container fluid className="my-4 px-2">
            ...
            <>
              <Row className="mt-3 mb-2 align-items-center">
                ...
                >
                  ...
                  >
                    {t('reportPage.backFromColumnDrillDown')}
                  </Button>)}
                </Col>
              </Row>
              ...
              >
                ...
                filterData[0].geographicLevel === 'structure' ? (
                    <Card className="text-start p-3">
                      ...
                      <small>
                        Percent of structures sprayed over total
                        - {irsOaProgressBar.completeStructuresPercent.toFixed(2)}%
                      </small>
                      <ProgressBar
                      ...
                      <small>
                        Percent of structures found over total
                        - {irsOaProgressBar.foundCoveragePercent.toFixed(2)}%
                      </small>
                      <ProgressBar
                      ...
                      <small>
                        Percent of Spray Coverage of Found(Sprayed/Found)
                        - {irsOaProgressBar.sprayCoveragePercent.toFixed(2)}%
                      </small>
                      <ProgressBar
                      ...
                      <p className="mb-1">
                        {irsOaProgressBar?.notSprayedStructures} of {filterData.length} structures
                        not
                        sprayed
                      </p>
        ...
        {showGraphs && graphData  ? <Row className="my-3 align-items-center">
          <Col md={showMap ? 10 : 2}>
            <Collapse in={showMap}>
              <div    style={{
                display: 'flex',
              ...
              />
                ...
                <div style={{width: '500px', height: '400px'}} className={'mp-2 mx-2'}>
        ...
        {showModal && currentFeature &&
            <ReportModal showModal={openModalHandler} feature={currentFeature}/>}
      </Container>
  );
};
